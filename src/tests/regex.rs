...
mod tests {
    ...
    fn test_complex_regex_pattern() -> ParsingResult<()> {
        ...
        assert!(matches!(tokens[7], RegexType::Quant(Quantifier::Range(0, 1))));
    ...
    #[test]
    fn test_add_concatenation_with_quantifiers() {
        // a*b should become a*·b
        let mut tokens = VecDeque::new();
        tokens.push_back(RegexType::Char('a'));
        tokens.push_back(RegexType::Quant(Quantifier::AtLeast(0))); // a*
        tokens.push_back(RegexType::Char('b'));
        ...
        assert!(matches!(result[0], TokenType::Literal(RegexType::Char('a'))));
        assert!(matches!(result[1], TokenType::UnaryOperator(RegexType::Quant(Quantifier::AtLeast(0)))));
        assert!(matches!(result[2], TokenType::BinaryOperator(RegexType::Concatenation)));
        assert!(matches!(result[3], TokenType::Literal(RegexType::Char('b'))));
    }

    #[test]
    fn test_add_concatenation_with_character_classes() {
        // [a-z]b should become [a-z]·b
        ...
        assert_eq!(result.len(), 3);
        if let TokenType::Literal(RegexType::Class(class)) = &result[0] {
            assert!(class.contains_char(&'a'));
    ...
    #[test]
    fn test_add_concatenation_with_dot() {
        // a.b should become a·.·b
        let mut tokens = VecDeque::new();
        tokens.push_back(RegexType::Char('a'));
    ...
    #[test]
    fn test_add_concatenation_multiple_chars() {
    ...
    #[test]
    fn test_add_concatenation_nested_groups() {
        ...
        assert!(matches!(result[2], TokenType::CloseParenthesis(RegexType::CloseParenthesis)));
        assert!(matches!(result[3], TokenType::BinaryOperator(RegexType::Concatenation)));
        assert!(matches!(result[4], TokenType::OpenParenthesis(RegexType::OpenParenthesis)));
        assert!(matches!(result[5], TokenType::Literal(RegexType::Char('b'))));
        assert!(matches!(result[6], TokenType::CloseParenthesis(RegexType::CloseParenthesis)));
        assert!(matches!(result[7], TokenType::BinaryOperator(RegexType::Concatenation)));
        assert!(matches!(result[8], TokenType::OpenParenthesis(RegexType::OpenParenthesis)));
        assert!(matches!(result[9], TokenType::Literal(RegexType::Char('c'))));
        assert!(matches!(result[10], TokenType::CloseParenthesis(RegexType::CloseParenthesis)));
    }

    #[test]
    fn test_add_concatenation_mixed_operators() {
        // a|bc should become a|b·c (no concatenation between a and |)
        let mut tokens = VecDeque::new();
        tokens.push_back(RegexType::Char('a'));
        tokens.push_back(RegexType::Or);
        tokens.push_back(RegexType::Char('b'));
    ...
    #[test]
    fn test_add_concatenation_complex_expression() {
        // (a|b)cd should become (a|b)·c·d
        let mut tokens = VecDeque::new();
        tokens.push_back(RegexType::OpenParenthesis);
        tokens.push_back(RegexType::Char('a'));
        tokens.push_back(RegexType::Or);
        ...
        tokens.push_back(RegexType::CloseParenthesis);
        tokens.push_back(RegexType::Char('c'));
        tokens.push_back(RegexType::Char('d'));
        ...
        assert!(matches!(result[3], TokenType::Literal(RegexType::Char('b'))));
        assert!(matches!(result[4], TokenType::CloseParenthesis(RegexType::CloseParenthesis)));
        assert!(matches!(result[5], TokenType::BinaryOperator(RegexType::Concatenation)));
        assert!(matches!(result[6], TokenType::Literal(RegexType::Char('c'))));
        assert!(matches!(result[7], TokenType::BinaryOperator(RegexType::Concatenation)));
        assert!(matches!(result[8], TokenType::Literal(RegexType::Char('d'))));
    }

    #[test]
    fn test_add_concatenation_empty_input() {
    ...
    #[test]
    fn test_add_concatenation_with_line_conditions() {
        // ^a$ should not add concatenation with ^ and $
        let mut tokens = VecDeque::new();
        ...
        assert!(matches!(result[1], TokenType::Literal(RegexType::Char('a'))));
        assert!(matches!(result[2], TokenType::StartOrEndCondition(RegexType::LineEnd)));
    }

    #[test]
    fn test_add_concatenation_with_escaped_chars() {
        // a\nb should become a·\n·b
        let mut tokens = VecDeque::new();
        tokens.push_back(RegexType::Char('a'));
        ...
        assert!(matches!(result[0], TokenType::Literal(RegexType::Char('a'))));
        assert!(matches!(result[1], TokenType::BinaryOperator(RegexType::Concatenation)));
        assert!(matches!(result[2], TokenType::Literal(RegexType::Char('\n'))));
        assert!(matches!(result[3], TokenType::BinaryOperator(RegexType::Concatenation)));
        assert!(matches!(result[4], TokenType::Literal(RegexType::Char('b'))));
    }

    #[test]
    fn test_add_concatenation_consecutive_quantifiers() {
        // This shouldn't happen in valid regex, but testing how it's handled
        // a*+ should be handled as a*·+ (though this is an invalid regex)
        let mut tokens = VecDeque::new();
        tokens.push_back(RegexType::Char('a'));
        ...
        // We're not testing the validity, just how the concatenation algorithm handles it
        assert_eq!(result.len(), 3);
        assert!(matches!(result[0], TokenType::Literal(RegexType::Char('a'))));
        assert!(matches!(result[1], TokenType::UnaryOperator(RegexType::Quant(Quantifier::AtLeast(0)))));
    ...
    #[test]
    fn test_add_concatenation_edge_case_patterns() {
        // Test a pattern with multiple nested groups and alternations
        ...
        assert!(matches!(result[4], TokenType::CloseParenthesis(RegexType::CloseParenthesis)));
        assert!(matches!(result[5], TokenType::BinaryOperator(RegexType::Concatenation)));
        assert!(matches!(result[6], TokenType::OpenParenthesis(RegexType::OpenParenthesis)));
        assert!(matches!(result[7], TokenType::Literal(RegexType::Char('c'))));
        assert!(matches!(result[8], TokenType::BinaryOperator(RegexType::Or)));
        assert!(matches!(result[9], TokenType::Literal(RegexType::Char('d'))));
        assert!(matches!(result[10], TokenType::CloseParenthesis(RegexType::CloseParenthesis)));
    }
    ...
    fn test_regex_new_with_complex_pattern() -> ParsingResult<()> {
        ...
        assert!(matches!(result[4], TokenType::Literal(RegexType::Char('c'))));
        assert!(matches!(result[5], TokenType::UnaryOperator(RegexType::Quant(Quantifier::Range(0, 1)))));
        assert!(matches!(result[6], TokenType::BinaryOperator(RegexType::Concatenation)));
